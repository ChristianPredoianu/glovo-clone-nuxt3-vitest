...
<template>
  <header class="relative bg-white border-b border-green-500">
    <nav class="flex items-center justify-between container mx-auto p-4">
      ...
      <Logo />
      <Backdrop :isBackdropOpen="isNavOpen" @closeElement="closeNav" />
      <Hamburger />
      <div
        id="nav-list-div"
        :class="isNavOpen ? 'translate-x-0' : '-translate-x-full'"
        class="h-full bg-white sm:bg-transparent fixed sm:relative z-50 top-0 left-0 w-10/12 sm:w-auto min-h-screen sm:min-h-full flex flex-grow flex-col items-center justify-center sm:justify-start sm:flex-row rounded-r-2xl sm:rounded-r-none transition ease-in-out sm:py-0 sm:translate-x-0 sm:ml-4"
      >
        <NavItems />
